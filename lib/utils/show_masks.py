...
def show_masks(im, detections, masks, class_names, cfg, scale=1.0, show = True):
    ...
    :param im: array of image [b=1 c h w] in rgb
    :param detections: [ numpy.ndarray([[x1 y1 x2 y2 score]]) for j in classes ]
    :param class_names: list of names in imdb
    :param scale: visualize the scaled image
    :return: array of image with differenet color masks
    """
    plt.cla()
    plt.axis("off")
    plt.imshow(im)
    ...
    for j, name in enumerate(class_names):
        ...
        for det, msk in zip(dets, msks):
            ...
            score = det[-1]
            cnt += 1
            plt.gca().text((bbox[0]) / 1, bbox[1], '{:s} {:.3f}'.format(name+'_'+str(cnt), score),
