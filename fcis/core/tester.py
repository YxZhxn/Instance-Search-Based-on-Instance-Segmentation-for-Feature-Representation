...
def conv_detect(predictor, data_batch, data_names, scales, cfg):
    """
    the difference between 'conv_detect' and 'im_detect' is that features of conv_new_1 are returned for later
    roi-pooling using.
    :param predictor:
    :param data_batch:
    :param data_names:
    ...
    for i in range(len(cfg.feature)):
        ...
        convs.append(lst)
        if cfg.feature[i] == 2:
            # flags.append('res2c_relu_output')
            flags.append('stage1_unit3_relu_output')
        elif cfg.feature[i] == 3:
            # flags.append('res3b3_relu_output')
            flags.append('stage2_unit4_relu_output')
        elif cfg.feature[i] == 4:
            # flags.append('res4b22_relu_output')
            flags.append('stage3_unit23_relu_output')
        elif cfg.feature[i] == 5:
            # flags.append('res5c_relu_output')
            flags.append('stage4_unit3_relu_output')
        elif cfg.feature[i] == 6:
            flags.append('relu_new_1_output')
    ...
    for output, data_dict, scale in zip(output_all, data_dict_all, scales):
        ...
        for i in range(len(flags)):
            convs[i].append(output[flags[i]].asnumpy())
        """"""
    ...
    return scores_all, pred_boxes_all, pred_masks_all, convs, data_dict_all
        ...
        for data_batch in test_data:
            ...
            for delta, (scores, boxes, masks, data_dict) in enumerate(zip(scores_all, boxes_all, masks_all, data_dict_all)):
                ...
                if not cfg.TEST.USE_MASK_MERGE:
                    ...
                    if mask_voting == gpu_mask_voting:
                        result_mask, result_box = mask_voting(masks, boxes, scores, imdb.num_classes,
                                                              ...
                                                              cfg.BINARY_THRESH, int(cfg.gpus.split(',')[0]))
                    else:
                        result_mask, result_box = mask_voting(masks, boxes, scores, imdb.num_classes,
                                                              max_per_image, im_width, im_height,
                                                              cfg.TEST.NMS, cfg.TEST.MASK_MERGE_THRESH,
