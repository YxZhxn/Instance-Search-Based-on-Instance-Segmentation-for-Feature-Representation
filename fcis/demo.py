...
def main():
    ...
    # load demo data
    data = []
    dataset_dir   = '/../input/debug/'
    result_dir    = '/../output/fcis/debug/'
    model_prefix  = '/../model/fcis_coco'
    image_names   = os.listdir(cur_path + dataset_dir)

    target_size = config.SCALES[0][0]
    max_size = config.SCALES[0][1]
    for im_name in image_names:

        assert os.path.exists(cur_path + dataset_dir + im_name), ('%s does not exist'.format(dataset_dir + im_name))
        im = cv2.imread(cur_path + dataset_dir + im_name, cv2.IMREAD_COLOR | cv2.IMREAD_IGNORE_ORIENTATION)
    ...
    arg_params, aux_params = load_param(cur_path + model_prefix, 0, process=True)
    ...
    for i in xrange(1):
        data_batch = mx.io.DataBatch(data=[data[0]], label=[], pad=0, index=0,
                                     ...
                                     provide_label=[None])
        scales = [data_batch.data[i][1].asnumpy()[0, 2] for i in xrange(len(data_batch.data))] # scales from im_info
    ...
    for idx, im_name in enumerate(image_names):
        ...
        im_shapes = [data_batch.data[i][0].shape[2:4] for i in xrange(len(data_batch.data))] #shapes from im_tensor
        ...
        elif im_width > 1024 or im_height > 1024:
            """
            bug: if the image is too large, which might cause a memory bug in 'gpu_mask_voting'. 
        ...
        im = cv2.imread(cur_path + dataset_dir + im_name)
        im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
        # im = show_masks(im, dets, masks, classes, config)
        im = save_results(cur_path + result_dir + im_name.replace('.jpg', '.dtc.jpg'), im, dets, masks, classes, config)
        cv2.imwrite(cur_path + result_dir + im_name.replace('.jpg', '.msk.jpg'), im)
