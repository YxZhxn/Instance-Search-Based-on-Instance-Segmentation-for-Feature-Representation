...
from nms.nms import gpu_nms_wrapper, cpu_nms_wrapper
...
class ProposalOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        # nms = cpu_nms_wrapper(self._nms_thresh)

        batch_size = in_data[0].shape[0]
        if batch_size > 1:  # only one image at a time
        ...
        pre_nms_top_n = self._rpn_pre_nms_top_n    # generate pre_nms_top_n rpns in one single image 6000
        post_nms_top_n = self._rpn_post_nms_top_n  # reserve post_nms_top_n rpns from one single image 300
        ...
        order = scores.ravel().argsort()[::-1]  # -1 to reverse the order list
        if pre_nms_top_n > 0:
